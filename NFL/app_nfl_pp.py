...
def run_app(df: pd.DataFrame | None = None):
    ...
    # ---------- Implied EV Calculation (same pooled curve + 10% floor) ----------
    ...
    # pooled coefficients from training
    A, B, C = -33.854, 0.3313, 0.0001
    def apply_vig_curve(o):
        m = abs(o)
        vig = max(A + B*m + C*(m**2), 0.10*m)  # 10% floor
    ...
    def calc_implied_ev(row):
        try:
            ...
            # vig-adjust other books -> fair odds -> to probs
            fair_probs = []
            for o in others:
                fair = apply_vig_curve(o)
